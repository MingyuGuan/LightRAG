...
class TestCase:
    ...
    def reset_engine(self, config: GraphLoomConfig):
        self.engine.engine.finalize_storages()
        del self.engine
        self.engine = RAGEngine(config)
        self.engine.engine.initialize_storages()
...
class SQualityTest(Test):
    ...
    def parse(self) -> List[TestCase]:
        ...
        for test in self.tests:
            test.engine.engine.initialize_storages()
    ...
    async def a_evaluate(self, test_case: TestCase) -> Result:
        ...
        test_case.engine.engine.finalize_storages()
